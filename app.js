...
function currentRows() {
  ...
  return sales.filter(s => s.date>=from && s.date<=to && (selectedChannel==='all' || String(s.ecommerce).toUpperCase().replace(/\s/g,'')===selectedChannel) && (!term || `${s.product} ${s.sku}`.toLowerCase().includes(term)));
}
...
function render() {
  ...
  qs('#period-label').textContent=`${period.label} · ${channelLabel} · ${rows.length ? `${localDate(rows.map(x=>x.date).sort()[0]).toLocaleDateString('pt-BR')} a ${localDate(rows.map(x=>x.date).sort().at(-1)).toLocaleDateString('pt-BR')}` : 'sem registros'}`;
