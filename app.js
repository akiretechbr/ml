...
function currentRows() {
  ...
  return sales.filter(s => s.date>=from && s.date<=to && (!term || `${s.product} ${s.sku}`.toLowerCase().includes(term)));
}
...
function render() {
  ...
  qs('#period-label').textContent=`${period.label} · ${rows.length ? `${localDate(rows.map(x=>x.date).sort()[0]).toLocaleDateString('pt-BR')} a ${localDate(rows.map(x=>x.date).sort().at(-1)).toLocaleDateString('pt-BR')}` : 'sem registros'}`;
  ...
  qs('#sales-table').innerHTML=grouped.length?grouped.map(x=>`<tr><td class="product">${escapeHtml(x.product)}</td><td>${escapeHtml(x.sku)}</td><td>${number.format(x.quantity)}</td><td><span class="pill return">${x.profitReturn.toLocaleString('pt-BR',{maximumFractionDigits:1})}%</span></td><td>${escapeHtml(x.ecommerce)}</td><td><span class="pill">${escapeHtml(x.log)}</span></td><td>${money.format(x.total)}</td></tr>`).join(''):'<tr><td class="empty" colspan="7">Nenhuma venda encontrada para este período.</td></tr>';
}
